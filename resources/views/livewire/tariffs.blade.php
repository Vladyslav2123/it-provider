<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="text-center mb-12">
        <h2 class="text-3xl font-extrabold cyber-heading sm:text-4xl">Наші тарифи</h2>
        <p class="mt-4 text-lg cyber-text">Виберіть план, який найкраще відповідає вашим потребам.</p>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        @forelse($tariffs as $tariff)
            <div class="cyber-card rounded-xl overflow-hidden transition-all hover:shadow-xl transform hover:scale-105 transition-transform duration-300">
                @if($tariff->is_popular)
                    <div class="bg-pink-600 text-white text-center py-2 font-semibold">
                        Найпопулярніший
                    </div>
                @endif
                <div class="p-6 {{ $tariff->is_popular ? '' : 'pt-10' }}">
