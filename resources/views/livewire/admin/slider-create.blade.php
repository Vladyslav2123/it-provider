    ...
    <form wire:submit.prevent="save" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" id="button_text" wire:model.defer="button_text"
                       class="admin-dark-input mt-1 block w-full rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                @error('button_text') <span class="text-red-400 text-sm">{{ $message }}</span> @enderror
            </div>

            <div>
                <label for="button_url" class="block text-sm font-medium">URL кнопки</label>
                <input type="text" id="button_url" wire:model.defer="button_url"
                       class="admin-dark-input mt-1 block w-full rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                @error('button_url') <span class="text-red-400 text-sm">{{ $message }}</span> @enderror
            </div>
        </div>
        ...
        <div class="flex justify-end">
            <button type="submit" class="admin-dark-button px-4 py-2 rounded hover:bg-blue-600">
                <i class="fas fa-save mr-2"></i> Створити слайдер
            </button>
        </div>
    </form>
</div>
